...
index.html ─── carrega ──▶ src/main.js
   ...
   ┌──────────┼──────────────┬───────────────┬──────────────┐
   ▼          ▼              ▼               ▼              ▼
 Input.js  Autopilot.js  entities/Tractor  entities/Truck  ui/HUD + ui/QuizModal
...
3. Liga o jogo em **modo demo** (`state.isDemo = true`, banner visível) e faz o wiring
   dos botões: "Assumir Comando" (→ `game.beginPlayerShift`), "Finalizar Turno"
   (→ `game.finishShift`), mute e colapso do painel de controles.
...
5. **Spawn de caminhões** enquanto `batch < maxBatch` (demo: 1; jogador: 5), um a cada
   2 s, alternando COLETOR/CARRETA. Quando todos terminam → `phase = 'READY'`.
6. Update do trator com o input ativo (jogador, ou `Autopilot` na demo). Quando o
   autopilot chega a `DONE`, `beginPlayerShift()` faz o handover automático.
...
12. `Enter` (fora da demo, em `READY`/`WAITING_TRUCKS` — as mesmas fases do botão
...
`state.isDemo` diferencia demo de turno do jogador (coach, segurança, relógio, diesel
e KPIs só valem para o jogador).
...
Tipos (em `params.js`): **COLETOR** (pilha ~10 m³ ≈ 2 viagens de lâmina),
**CARRETA** (maior, pilha ~20 m³ **mais larga que a lâmina** — força o ataque 2/3|1/3)
e **DEMO** (pilha curta de 4,5 m³ para um ciclo-modelo rápido). Enquanto manobra ou
...
## Autopilot (`engine/Autopilot.js`)

Piloto da demonstração — executa o ciclo TEC "de livro" e narra cada técnica no rádio
(cada mensagem uma vez, via `announced`). Alimenta o trator com um **input sintético**
(mesma interface do `Input`), então a física usada é exatamente a do jogador.

Máquina de estados:

```
SCAN ──▶ REPOSITION ──▶ ALIGN ──▶ PUSH ──▶ SPREAD ──▶ (CLIMB) ──▶ PENDULUM ─┐
 ▲                                              │(crista c/ carga)          │
 │                                              ▼                           ▼
 └──────────────◀─────────────────────────── RETURN ◀───────────────────────┘
 │
 └─(sem lixo solto)──▶ COMPACT_NEXT ─▶ COMPACT_GOTO ─▶ COMPACT_ALIGN ─▶ COMPACT_UP ⇄ COMPACT_DOWN
                              │ (sem telhas restantes, até 3 rodadas de replanejamento)
                              ▼
                            DONE  → Game faz o handover para o jogador
```

Decisões notáveis:
- **SCAN** procura a pilha mais à esquerda (TEC 7 — sentido único); pilha mais larga
  que a lâmina → ataca pela borda com 1/3 livre (TEC 3); estreita → centro de massa.
- **Staging** fora da praça (abaixo do material), chegando já alinhado à faixa.
- **RETURN** desce de ré com ½ esteira de deslocamento **alternando o lado** por
  viagem (o "passo de dança"), lâmina a 10 cm só na rampa (na praça viaja erguida).
- **Acabamento** ladrilha as colunas subcompactadas em telhas de 4 colunas
  (`planCompactTiles`), mede o terreno (`lanePasses`) e repete o ciclo até a pior
  célula entrar na faixa verde (teto de 4 ciclos contra super compactação).
- **Watchdog**: nenhum estado pode durar 25 s; se travar, volta a SCAN (e desiste
  para DONE após 3 trips).
- Com a lâmina carregada, o esterço é em **pulsos curtos** (respeita a regra da
  curva em carga).

## Input (`engine/Input.js`)

Mapa de `e.code` → bool com snapshot do frame anterior: `isDown` e `isJustPressed`.
`Game.loop` chama `input.update()` no fim do frame. (O autopilot implementa a mesma
interface com um objeto simples.)
...
| Script | Papel |
|---|---|
| `validate_demo.js` | Valida o ciclo completo da demo (autopilot) |
| `validate_player.js` | "Operador perfeito": reutiliza o autopilot **como input do jogador** (coach, segurança e quizzes valendo) num turno de 5 caminhões e imprime KPIs/bandas finais |
| `diagnose_freeze.js`, `debug_test.js`, `test*.js` | Diagnóstico/experimentos |
